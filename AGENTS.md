...
| Invoice | `/api/v1/invoices/**` | ADMIN/STAFF lấy danh sách phân trang hoặc tra cứu theo `GET /{invoiceId}`; user lấy `/my` và chỉ đọc hóa đơn của mình. `InvoiceDTO` trả `subtotalAmount`, `discountAmount`, `shippingFee`, `totalAmount`, `province`, `district`, `ward`, `shippingAddress` và `order`; payment gọi riêng theo `orderId`. |
...
| 2026-07-18 | Tra cứu hóa đơn | Bổ sung ô tra cứu mã hóa đơn tại Admin, gọi đúng `GET /api/v1/invoices/{invoiceId}`; đồng bộ tài liệu theo `InvoiceDTO` BE mới có breakdown tiền và ba cấp địa chỉ. |
| 2026-07-18 | Quản lý trạng thái user | Khi tài khoản có `isActive=false`, FE hiển thị thao tác Mở khóa và gọi `PUT /api/v1/users/{username}` với `{ isActive: true }`; tài khoản hoạt động tiếp tục có thao tác Khóa. Cấp STAFF/ADMIN được chuyển từ bảng vào modal Sửa tài khoản; quyền BE là enum đơn (`USER`/`STAFF`/`ADMIN`), FE có thao tác đặt lại `USER` và không hiện cấp STAFF cho ADMIN. |
