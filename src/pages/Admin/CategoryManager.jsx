...
import { isAdmin, isAdminOrStaff } from '../../utils/auth.js'

export const CategoryManager = () => {
  const canManageCategoryLifecycle = isAdmin()
  const canRestoreCategory = isAdminOrStaff()
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      if (modalMode === 'create') {
        if (!canManageCategoryLifecycle) {
          toast.error('Chỉ quản trị viên được tạo danh mục mới.')
          return
        }
        const payloadCreate = {
        ...
        if (!canManageCategoryLifecycle) {
          toast.error('BE mới chỉ cho ADMIN cập nhật danh mục.')
          return
        }
        // Edit mode
                    ...
                    >
                      ...
                      <td className="py-4.5 px-6 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          {cat.isActive ? (canManageCategoryLifecycle && (
                              <button
